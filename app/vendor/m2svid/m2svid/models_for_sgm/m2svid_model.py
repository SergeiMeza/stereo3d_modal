...
class VideoLDM(DiffusionEngine):
    def __init__(
        ...
        # [vendored patch] upstream instantiates LPIPS() here, which
        # downloads VGG weights from external hosts at model init. LPIPS
        # is only used by test_step/validation_step (training/eval), so
        # it is dropped for inference-only deployment.
    ...
    def init_from_ckpt(
        ...
        # [vendored patch] torch>=2.6 defaults weights_only=True, which
        # rejects the deepspeed-format release checkpoint. The weights
        # come from a pinned Google-hosted URL, so loading is trusted.
