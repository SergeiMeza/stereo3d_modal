...
class VideoDepthAnything(nn.Module):
    ...
    def infer_video_depth(
        ...
        _, frame_height, frame_width = frames[0].shape
        ...
        resize_height = round(frame_height / 14) * 14
        resize_width = round(frame_width / 14) * 14

        if max(resize_height, resize_width) > max_res:
            scale = max_res / max(frame_height, frame_width)
            resize_height = round(frame_height * scale / 14) * 14
            resize_width = round(frame_width * scale / 14) * 14

        print(f"==> infer lower bound input size HxW: {resize_height}x{resize_width}")
        ...
        for frame_id in tqdm(range(0, org_video_len, frame_step)):
            ...
            cur_input = transform(
                torch.stack(frame_list[frame_id : frame_id + INFER_LEN], dim=0)
                .to(device)
                .unsqueeze(0)
                / 255.0
            )
            ...
            depth: torch.Tensor = F.interpolate(
